...
class AttrLengthCorruptionCondition(AccountCorruptionConditionInterface):
    def isCorrupted(self, account : Account) -> bool:
        return isEven(len(account.__dict__))
...
class Bank(object):
    ...
    def transfer(self, origin, dest, amount) -> bool:
        ...
        if any(None for i in [origin_account, dest_account]) or \
            any(self.corruption_inspector.isCorrupted(i) == True for i in [origin_account, dest_account]) or \
